...
function ArticleCard({ post, onOpen = () => {} }) {
  return (
    <article className="ik-article-card" onClick={() => onOpen(post)}>
      <a className="ik-article-card__cover" href="#" onClick={(e) => { e.preventDefault(); onOpen(post); }} aria-hidden="true">
        <div className="ik-article-card__cover-fill" style={{ background: post.cover.bg }}>
      ...
      <h3 className="ik-article-card__title">
        <a href="#" onClick={(e) => { e.preventDefault(); onOpen(post); }}>{post.title}</a>
      </h3>
