...
function CodeBlock({ children }) {
  ...
  return (
    <div className="ik-codeblock">
      <button className="ik-codeblock__copy" onClick={onCopy}>{copied ? "Copied" : "Copy"}</button>
  ...
  return (
    <div className="ik-article-body">
      {blocks.map((b, i) => {
        ...
        if (b.kind === "ol") return <ol key={i}>{b.items.map((x, j) => <li key={j}>{x}</li>)}</ol>;
        if (b.kind === "ul") return <ul key={i}>{b.items.map((x, j) => <li key={j}>{x}</li>)}</ul>;
        if (b.kind === "pre") return <CodeBlock key={i}>{b.text}</CodeBlock>;
        if (b.kind === "callout") return (
          <Callout key={i} variant={b.variant} title={b.title}>{b.text}</Callout>
        );
        return null;
      })}
    </div>
  ...
  return (
    <article className="ik-article container-narrow">
      <div className="ik-article__crumbs">
        <a href="#" onClick={(e) => { e.preventDefault(); onBack(); }}>← All writing</a>
      </div>
