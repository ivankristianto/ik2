...
- All static pages follow the post-content model: an aggregate pattern (`ik2/home-page`, `ik2/resume-page`, `ik2/about-page`, `ik2/contact-page`, `ik2/speaking-page`) lists leaf `wp:pattern` refs; the page template renders `<main class="ik-<page>">` wrapping a `wp:post-content` block (align full, constrained, contentSize var full); the page's post_content (home 1882, about 46, contact 47, speaking 49, resume 50) holds the expanded markup, regenerated via a tmp `wp eval-file` script (parse → `resolve_pattern_blocks` → strip `metadata.patternName` → `wp_slash` + `wp_update_post`). The contact aggregate keeps its plain flow shield group; the speaking aggregate carries the dynamic `ik2/speaking-archive` block in post_content, which is fine.
